...
export const useGameStore = create<GameState>()(
    persist(
        (set, get) => ({
            ...
            setChallengeHours: (challengeHours) => set({ challengeHours }),
            setChallengeReward: (challengeReward) => set({ challengeReward }),
            ...
            startSession: async (words, challengeHours = 24, challengeReward = 35) => {
                ...
                const now = Date.now();
                const unlockMs = now + challengeHours * 60 * 60 * 1000;
            ...
            submitRecall: async (answers, score) => {
                ...
                // Use challenge reward stored on session, fallback to 15
                const nousReward = (session as any).nous_reward ?? 15;
                const allWordsCorrect = answers.length > 0 && score >= session.words.length * (session.words.length <= 3 ? 50 : session.words.length <= 9 ? 30 : 20);
                const nousEarned = (success && allWordsCorrect) ? nousReward : 0;
                ...
                await supabase
                    .from('profiles')
                    .update({
                        nous_coins: (profile?.nous_coins ?? 0) + nousEarned,
